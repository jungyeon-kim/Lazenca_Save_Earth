	...
	wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
		...
		case 1:
			...
			ItemTimeCount++;
			if (ItemTimeCount % 4900 == 0)
				I_power.emplace_back(&Item[2]);
			if (ItemTimeCount % 5900 == 0)
				I_skill.emplace_back(&Item[3]);
			if (ItemTimeCount % 2250 == 0)
	...
	case WM_PAINT:
		...
		scene.draw(memdc, MapY1, MapY2, WndX, WndY, FinalScore);
...
void SetAllTimer(HWND hwnd)
{
	...
	SetTimer(hwnd, 2, 150, NULL); // 플레이어 총알 생성
	SetTimer(hwnd, 3, 3000, NULL); // 적 비행기1 생성
	SetTimer(hwnd, 4, 8000, NULL); // 적 비행기2 생성
	SetTimer(hwnd, 5, 1500, NULL); // 적 총알 생성
	SetTimer(hwnd, 6, 2000, NULL); // 적 총알2 생성
	SetTimer(hwnd, 7, 20000, NULL); // 적 비행기3 생성
	SetTimer(hwnd, 8, 1500, NULL); // 적 총알3 생성
	...
	SetTimer(hwnd, 13, 2000, NULL); // 보스 총알3 생성
	SetTimer(hwnd, 14, 2500, NULL); // 보스 총알3 - 1 생성
